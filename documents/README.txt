...
2. Detects beacons nearby the users -> I could use a beacon which was supported by Estimote by using a library.
				 I could confirm that the application could detect beacons nearby.
3. Sends device's uuid and beacon's id to the beacon.
				-> By registering to the beacon the device, the communication between the beacon and the device becomes available.
